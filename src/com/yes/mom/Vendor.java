...
/**
 * The Vendor synchronously, and in a single transaction, receives the order
 * from VendorOrderQueue and sends messages to the two Suppliers via
 * MonitorOrderQueue and StorageOrderQueue. The responses are received
 * asynchronously; when both responses come back, the order confirmation message
 * is sent back to the Retailer.
 */
public class Vendor implements Runnable, MessageListener {
	...
	public void run() {
		...
		try {
			...
			asyncSession = asyncconnection.createSession(true,
					Session.SESSION_TRANSACTED);

			vendorConfirmQueue = asyncSession.createTemporaryQueue();
			MessageConsumer confirmConsumer = asyncSession
			...
			while (true) {
				...
				try {
					...
					if (inMessage instanceof MapMessage) {
						...
						session.commit();
						System.out.println("----------vendor end----------");
						break;
					}

					// Randomly throw an exception in here to simulate a
					// Database error
					...
					System.out.println("Vendor: JMSException Occured: "
							+ e.getMessage());
					e.printStackTrace();
					session.rollback();
	...
	public void onMessage(Message message) {
		...
		try {
			...
			if (!"Pending".equals(order.getStatus())) {
				System.out
						.println("Vendor: Completed processing for order --No."
								+ orderNumber);

				MessageProducer replyProducer = asyncSession
