...
/**
 * The Supplier synchronously receives the order from the Vendor and randomly
 * responds with either the number ordered, or some lower quantity.
 */
public class Supplier implements Runnable {
	...
	public Supplier(String item, String queue, String url, String user,
			String password) {
		this.url = url;
		this.user = user;
	...
	public void run() {
		...
		try {
			...
			while (true) {
				...
				System.out.println(ITEM + " Supplier: Vendor ordered "
						+ quantity + " " + orderMessage.getString("Item")
						+ "--No." + orderMessage.getInt("VendorOrderNumber"));
				...
				quantity = Math
						.min(orderMessage.getInt("Quantity"), new Random()
								.nextInt(orderMessage.getInt("Quantity") * 10));
				outMessage.setInt("Quantity", quantity);

				producer.send(outMessage);
				System.out.println(ITEM + " Supplier: Sent " + quantity + " "
						+ ITEM + "(s)");
				session.commit();
	...
	public static void main(String[] args) {
		...
		if ("HardDrive".equals(item)) {
			...
			throw new IllegalArgumentException(
					"Item must be either HardDrive or Monitor");
		}
